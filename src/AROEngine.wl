...
(* ============================================================================
   AROEngine.wl - Harmony-Guided Adaptive Graph Optimization Engine
   ============================================================================
...
BeginPackage["IRHSuite`AROEngine`"];

AROEngine::usage = "AROEngine[initGraph, opts] runs the ARO optimization loop. \
...
Example: result = AROEngine[g, \"MaxIterations\" -> 1000]";

AROStep::usage = "AROStep[state] performs a single ARO iteration. \
Used internally by AROEngine but exposed for debugging.";
...
AROEngine[initGraph_?GraphStateQ, opts:OptionsPattern[]] := Module[
...
AROEngine[_, ___] := (
  Message[AROEngine::invalidgs];
  $Failed
);

AROEngine::invalidgs = "AROEngine requires a valid GraphState as input.";
...
AROStep[state_Association] := Module[
